...
//DEFINE_LOG_CATEGORY(UseXFLog);//定义一个log标签
...
UseXF::UseXF(FString s)
{
	//UE_LOG(UseXFLog, Warning, TEXT("UseXF(FString) 调用"));
}
...
//public 方法
int UseXF::get_default_input_dev()
{
	return WAVE_MAPPER;
}

unsigned int UseXF::get_input_dev_num()
{
	return waveInGetNumDevs();
}

//回调将在一个新线程上运行
int UseXF::create_recorder(recorder ** out_rec, void(*on_data_ind)(char *data, unsigned long len, void *user_para), void * user_cb_para)
{
...
void UseXF::destroy_recorder(recorder * rec)
{
	if (!rec)
		return;
...
int UseXF::open_recorder(recorder * rec, unsigned int dev, WAVEFORMATEX * fmt)
{
	int ret = 0;
	if (!rec)
...
void UseXF::close_recorder(recorder * rec)
{
...
int UseXF::start_record(recorder * rec)
{
	int ret;
	if (rec == NULL)
		return -RECORD_ERR_INVAL;
...
int UseXF::stop_record(recorder * rec)
{
	int ret;
	if (rec == NULL)
		return -RECORD_ERR_INVAL;
...
int UseXF::is_record_stopped(recorder * rec)
{
	if (rec->state == RECORD_STATE_RECORDING)
		return 0;
...
void  UseXF::dbg_wave_header(WAVEHDR * buf)
{
	//Printf("-----\n");
...
int  UseXF::create_callback_thread(void *thread_proc_para, HANDLE *thread_hdl_out)
{
	int ret = 0;
...
void  UseXF::close_callback_thread(HANDLE thread)
{
	if (thread == NULL)
		return;
...
int  UseXF::open_rec_device(int dev, WAVEFORMATEX *format, HANDLE thread, HWAVEIN *wave_hdl_out)
{
	MMRESULT res;
	HWAVEIN wi = NULL;
	WAVEFORMATEX fmt;
...
int  UseXF::prepare_rec_buffer(HWAVEIN wi, WAVEHDR ** bufheader_out, unsigned int headercount, unsigned int bufsize)
{
	int ret = 0;
	unsigned int i = 0;
	char clearout = 0;
...
void  UseXF::free_rec_buffer(HWAVEIN wi, WAVEHDR *first_header, unsigned headercount)
{
	unsigned int i;
	WAVEHDR *header;
...
void  UseXF::close_rec_device(HWAVEIN wi)
{
	if (wi != NULL) {
		waveInClose(wi);
	}
...
int  UseXF::start_record_internal(HWAVEIN wi, WAVEHDR *header, unsigned int bufcount)
{
	MMRESULT res;
	unsigned int i;
...
int  UseXF::stop_record_internal(HWAVEIN wi)
{
	MMRESULT res;
...
void  UseXF::data_proc(struct recorder *rec, MSG *msg)
{
	HWAVEIN whdl;
	WAVEHDR *buf;
...
int  UseXF::is_stopped_internal(struct recorder *rec)
{
	unsigned int i;
	WAVEHDR *header;
...
int  UseXF::open_recorder_internal(struct recorder * rec, unsigned int dev, WAVEFORMATEX * fmt)
{
...
void  UseXF::close_recorder_internal(struct recorder *rec)
{
	if (rec->wavein_hdl) {
		close_rec_device((HWAVEIN)rec->wavein_hdl);
